		...
		try{
			...
			if(visualizationSQLresult.rows.length != 0)//run this code only if the project contains rows
			{
				...
				for(int i = 0; i < rows.length; i++)
				{
					...
					singleObject.queryObjectName= singleRow[0].toString();//query object title
					singleObject.finalQueryObject = singleRow[1];// the actual query object content (json)
					singleObject.projectDescription = singleRow[2].toString();//description of the project it belongs to
					
					//visualizations generated by executing a query object
					...
					//the author of the query object
					singleObject.author = singleRow[4].toString();
